...
- Condition: CLI cannot connect to `~/.cache/agent-search-gateway-cli/daemon.sock`.
- Handling: Print an instruction to start `agent-search-gateway start` to stderr and exit non-zero.
...
- `agent-search-gateway keyword-search ...`: stdout contains only the jsonl path.
- `agent-search-gateway llm-search ...`: stdout contains only the jsonl path.
- `agent-search-gateway url-fetch URL`: stdout contains content or unavailable message.
- `agent-search-gateway url-fetch URL FOCUS`: stdout contains focus summary or unavailable message.
- `agent-search-gateway stop`: stdout contains `Daemon stopped.` when it stopped a running daemon, or `Daemon is not running.` when no daemon was running.
