...
- `agent-search-gateway stop` sends a `shutdown` request over the configured socket.
...
- Daemon shutdown closes provider clients and removes `~/.cache/agent-search-gateway-cli/daemon.sock`.
...
- Missing daemon prints instruction to run `agent-search-gateway start` for workflow commands.
